...
async function checkRemoteAhead(dirname) {
	...
	if (dirty.includes(true)) {
		throw new Error(
			"Remote contains changes you don't have yet, please `git pull` before using nuup",
		);
	}
}
...
async function performActions(commands, options) {
	...
	if (options.isDefault) {
		return actions.runDefault({ dirname: options.dirname });
	}

	const promises = [];
	for (const a of commands) {
		promises.push(actions.run(a, { dirname: options.dirname }));
	}
...
async function cli(argv) {
	...
	if (commands.length > actions.max) {
		throw new Error(
			`Number of actions (${commands.length}) is more than allowed: ${actions.max}`,
		);
	}
	...
	return checkGitClean(dirname)
		...
		.then(() =>
			performActions(commands, {
				isDefault: commands.length === 0,
			}),
		)
		.catch((err) => spinner.fail(err.message));
}

cli(process.argv.slice(2)).catch((err) => spinner.fail(err.message));
