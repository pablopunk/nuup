...
const { log, error, happy } = require('./log')
...
function commitFile (dirname, file, version) {
  const cmd = `git add ${file} && git commit -m ${version} && git tag ${version} && git push`
...
async function checkTagsAndUpdateVersion ({
  ...
  if (!latestTag) {
    happy(`This is the first time you use ${pkg.name}, congrats!`)
  } else {
    const commits = await checkCommitsBetweenTags(current, dirname)
    if (commits.length === 0) {
      error(`There are no commits since version ${current}`)
      process.exit(1)
    }
  }
