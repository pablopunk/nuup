...
function commitFiles(state) {
	const cmd = `git add package* && git commit -m ${state.newVersion} && git tag ${state.newVersion} && git push && git push --tags`;

	return execa.shell(cmd, { cwd: state.options.dirname }).then(() => state);
}
